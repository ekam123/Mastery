...
struct User {
    ...
    mutating func deleteGoalbyID ( goalID : String) {
        ...
        if (allElements[goalID] as? Goal) != nil {
            ...
            for (taskID,_) in self.allTasksOfGoal(goalID: goalID)!
            {
                allElements.removeValue(forKey: taskID)
            }
            
            for (planID, _) in self.allPlansOfGoal(goalID: goalID)!
            {
                allElements.removeValue(forKey: planID)
            }
            
            allElements.removeValue(forKey: goalID)
            goalList = goalList?.filter { $0 != goalID }
    ...
    mutating func deletePlanbyID ( planID : String) {
        ...
        if (allElements[planID] as? Plan) != nil {
            ...
            for (taskID, _) in self.allTasksOfPlan(planID: planID)!
            {
                allElements.removeValue(forKey: taskID)
            }
...
func loadSampleUser() -> User {
    ...
    var allElements : Dictionary<String, Any> = [:]
    ...
    var amirCourse = Plan(id: UUID().uuidString,
                          ...
                          deadline: [Date.init(timeIntervalSinceNow: 15000)],
                          taskList: [t1.id, t2.id],
                          purpose: "Learn to make an app")
    
    allElements[amirCourse.id] = amirCourse
    ...
    allElements[lhl.id] = lhl
    
    var goal1 = Goal(id: UUID().uuidString,
                     name: "learn iOS",
                     purpose: "I want to be a programmer", isComplete: false,
                     dateOfBirth: Date.init(), deadline: [],
                     planList: [amirCourse.id, lhl.id])
    
    allElements[goal1.id] = goal1
    ...
    var run = Plan(id: UUID().uuidString,
                   ...
                   isComplete: false,
                   prerequisitePlans: nil,
                   deadline: [Date.init(timeIntervalSinceNow: 50000)],
                   taskList: [t5.id, t6.id],
                   purpose: "Be faster than a cheetah?")
    
    allElements[run.id] = run
    ...
    allElements[strengthTraining.id] = strengthTraining
    ...
    var stretch = Plan(id: UUID().uuidString,
                       ...
                       deadline: [Date.init(timeIntervalSinceNow: 238289372987)],
                       taskList: [t9.id, t10.id],
                       purpose: "be as nimble as a ballerina")
    
    allElements[stretch.id] = stretch
    
    var goal2 = Goal(id: UUID().uuidString,
                     name: "fitness",
                     purpose: "Become a beast", isComplete: false,
                     dateOfBirth: Date.init(), deadline: [],
                     planList: [run.id,
                                strengthTraining.id,
                                stretch.id])
    
    allElements[goal2.id] = goal2
